...
/**
 * Get the target extruder from the T parameter or the active_extruder 从 T 参数或 active_extruder 获取目标挤出机
...
/**
 * Get the target E stepper from the 'T' parameter.             从“T”参数获取目标 E 步进器。
 * If there is no 'T' parameter then dval will be substituted.  如果没有“T”参数，则将替换 dval。
 * Returns -1 if the resulting E stepper index is out of range. 如果生成的 E 步进器索引超出范围，则返回 -1。
 */
...
/**
 * Set XYZ...E destination and feedrate from the current GCode command  从当前 GCode 命令设置 XYZ...E 目标和进给率
 *
 *  - Set destination from included axis codes   - 从包含的轴代码设置目的地
 *  - Set to current for missing axis codes      - 设置为当前丢失的轴代码
 *  - Set the feedrate, if included              - 设置进给率，如果包括的话
 */
void GcodeSuite::get_destination_from_command() {
  ...
  if (parser.floatval('F') > 0) {
    ...
    // Update the cutter feed rate for use by M4 I set inline moves.  更新刀具进给率以供 M4 I 设置内联移动使用。
...
/**
 * Process the parsed command and dispatch it to its handler  处理已解析的命令并将其分派给它的处理程序
 */
void GcodeSuite::process_parsed_command(const bool no_ok/*=false*/) {
 ...
 /**
  * Block all Gcodes except M511 Unlock Printer, if printer is locked   如果打印机被锁定，则阻止除 M511 Unlock Printer 之外的所有 Gcode
  * Will still block Gcodes if M511 is disabled, in which case the printer should be unlocked via LCD Menu  如果禁用 M511，仍会阻止 Gcode，在这种情况下，应通过 LCD 菜单解锁打印机
  */
  #if ENABLED(PASSWORD_FEATURE)
  ...
  switch (parser.command_letter) {
    ...
    case 'T': T(parser.codenum); break;                           // Tn: Tool Change  工具改变
  ...
  SERIAL_OUT(msgDone); // Call the msgDone serial hook to signal command processing done调用 msgDone 串行挂钩以发出命令处理完成的信号
}
...
/**
 * Run a series of commands, bypassing the command queue to allow  运行一系列命令，绕过命令队列以允许从其他
 * G-code "macros" to be called from within other G-code handlers.  G 代码处理程序中调用 G 代码“宏”。
 */
